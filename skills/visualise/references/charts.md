...
- Wrap canvas in a container with explicit height and `position: relative`.
- Set `responsive: true` and `maintainAspectRatio: false`.
- Read CSS variables for text and border colors.
- Use ramp colors for series.
- Set `borderRadius: 4` on bars.
- Hide x-axis grid and show y-axis grid in border color.
- Use 12px chart text.
...
For line or area fills, use the same color at 20% opacity.

## Chart type selection

| Data shape | Type | Notes |
|-----------|------|-------|
| Categories and values | Bar | Use horizontal bars for long labels |
| Time series | Line | Set `tension: 0.3` |
| Parts of whole | Doughnut | Use `cutout: '60%'` |
| Two variables | Scatter | Add trend line if correlation matters |
...
For simple visuals that stream better:
...
For custom force graphs, maps, or treemaps:
...
Use D3 only when Chart.js cannot handle the layout.
...
- Sliders: set `step` for clean values
