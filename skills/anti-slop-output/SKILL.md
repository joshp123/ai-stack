...
description: "Write and edit every human-facing agent output in a GOV.UK / GDS anti-slop style: British English, plain English, active voice, front-loaded content, sentence case, and no bold or italics for emphasis."
---
...
Apply it to all human-facing agent output: chat responses, status updates, plans, reviews, reports, PR or issue text, docs, summaries, comments, emails and any prose meant to be read. When you write anything for a person, default to this style. When you brief another agent, pass this skill so its output follows the same style.

Use British English for agent-authored prose. Code, commands, paths, logs, identifiers, direct quotations, legal text and source material keep their exact wording and spelling.

## Content design principles

- Start from the user need. Write what the reader needs to know to do or decide something, not what you want to say.
- Front-load everything. Put the most important point first: in the output, each section, each paragraph and each sentence. Use the inverted pyramid: conclusion first, then detail, then background.
...
- Address the reader as "you". Write about yourself as "I" and about shared work as "we". Use "they", "them" and "their" rather than gendered pronouns. Write "disabled people", not "the disabled".
...
- Ampersands: write "and", not "&" (except in a registered name, logo or code).
...
- Is everything in British English and sentence case, with descriptive headings and links?
- Could you cut any more words without losing meaning? If yes, cut them.

## Note on this skill's own scope

The "no bold" and formatting rules apply to the prose you produce. Code, commands, paths, logs, identifiers, direct quotations, legal text, source material and data tables keep their own conventions. Markdown headings and lists are fine because they are structure, not emphasis.

Read `references/source-context.md` only when updating this skill, auditing its origin, or comparing it to the source tweets, gist and GOV.UK guidance. Do not load it for ordinary writing tasks.
