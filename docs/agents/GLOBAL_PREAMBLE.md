...
- **No global installs**: Use devenv.sh/devenv.nix per-project (`devenv --help`). Global config only via your private `nixos-config` repo (typically `$HOME/code/nix/nixos-config`).
